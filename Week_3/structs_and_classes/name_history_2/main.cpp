...
vector<string> FindAllNamesByYear(const map<int, string>& names, int year) {
  ...
  for (const auto& item : names) {
    // если очередной год не больше данного и имени нет в списке, добавляем имя
    // в список
    if (item.first <= year) {
      if (all_names.size() == 0) {
      ...
      // к будущему илиэ уже есть в списке
      break;
    }
  }
  return all_names;
...
class Person {
 public:
  ...
  string GetFullNameWithHistory(int year) {
    ...
    const vector<string> first_names_by_year =
        FindAllNamesByYear(first_names, year);
    const vector<string> last_names_by_year =
        FindAllNamesByYear(last_names, year);
    ...
    if (first_names_by_year.empty() && last_names_by_year.empty()) {
      ...
      return VectorToString(first_names_by_year) + " " +
             VectorToString(last_names_by_year);
    }
  }
